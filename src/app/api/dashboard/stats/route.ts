...
export async function GET(request: NextRequest) {
  try {
    ...
    const teacherCount = await db.prepare(`
      SELECT COUNT(*) as c FROM teachers t
      WHERE t.status = 'active' ${teacherSchoolParam ? 'AND t.school = ?' : ''}
    `).get(...(teacherSchoolParam ? [teacherSchoolParam] : [])) as any;

    const studentCount = await db.prepare(`
      SELECT COUNT(DISTINCT e.student_id) as c FROM enrollments e
      JOIN classes c ON e.class_id = c.id
      WHERE e.status = 'active' ${stageLikeMiddle ? 'AND c.grade LIKE ?' : ''}
    `).get(...(stageLikeMiddle ? [stageLikeMiddle] : [])) as any;

    const classCount = await db.prepare(`
      SELECT COUNT(*) as c FROM classes WHERE status = 'active' ${stageLikeMiddle ? 'AND grade LIKE ?' : ''}
    `).get(...(stageLikeMiddle ? [stageLikeMiddle] : [])) as any;
    ...
    const attendanceStats = await db.prepare(`
      ...
      WHERE 1=1 ${stageLikeMiddle ? 'AND c.grade LIKE ?' : ''}
    `).get(...(stageLikeMiddle ? [stageLikeMiddle] : [])) as any;
    ...
    const reportCounts = await db.prepare(`
      ...
      WHERE r.status = 'active' ${stageLikeMiddle ? 'AND c.grade LIKE ?' : ''}
      GROUP BY r.report_type
    `).all(...(stageLikeMiddle ? [stageLikeMiddle] : [])) as any[];

    const middleVsHigh = await db.prepare(`
      SELECT CASE WHEN grade LIKE '%ثانوي%' THEN 'الثانوية' ELSE 'المتوسطة' END as stage, COUNT(*) as c
      FROM classes WHERE status = 'active' ${stageLikeMiddle ? 'AND grade LIKE ?' : ''} GROUP BY stage
    `).all(...(stageLikeMiddle ? [stageLikeMiddle] : [])) as any[];
    ...
    `).all(...(stageLikeMiddle ? [stageLikeMiddle] : []));
    ...
    const scheduleStats = await db.prepare(`
      SELECT day_of_week, COUNT(*) as c
      FROM schedules s
      JOIN classes c ON s.class_id = c.id
      WHERE s.status = 'active' ${stageLikeMiddle ? 'AND c.grade LIKE ?' : ''}
      GROUP BY day_of_week
    `).all(...(stageLikeMiddle ? [stageLikeMiddle] : [])) as any[];

    const teacherWorkload = await db.prepare(`
      SELECT t.id, t.first_name, t.last_name, COUNT(s.id) as session_count
      FROM schedules s
      JOIN teachers t ON s.teacher_id = t.id
      JOIN classes c ON s.class_id = c.id
      WHERE s.status = 'active' ${stageLikeMiddle ? 'AND c.grade LIKE ?' : ''} ${teacherSchoolParam ? 'AND t.school = ?' : ''}
      GROUP BY t.id ORDER BY session_count DESC LIMIT 8
    `).all(...[...(stageLikeMiddle ? [stageLikeMiddle] : []), ...(teacherSchoolParam ? [teacherSchoolParam] : [])]) as any[];

    const subjectDistribution = await db.prepare(`
      SELECT sub.name as subject_name, COUNT(s.id) as session_count
      FROM schedules s
      JOIN subjects sub ON sub.name = s.subject
      JOIN classes c ON s.class_id = c.id
      WHERE s.status = 'active' ${stageLikeMiddle ? 'AND c.grade LIKE ?' : ''}
      GROUP BY sub.id ORDER BY session_count DESC LIMIT 8
    `).all(...(stageLikeMiddle ? [stageLikeMiddle] : [])) as any[];

    const hourlyDistribution = await db.prepare(`
      SELECT CAST(SUBSTR(s.start_time, 1, 2) AS INTEGER) as hour, COUNT(*) as c
      FROM schedules s
      JOIN classes c ON s.class_id = c.id
      WHERE s.status = 'active' ${stageLikeMiddle ? 'AND c.grade LIKE ?' : ''}
